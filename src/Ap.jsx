...
{showCoffee && (
  <div onClick={() => setShowCoffee(false)} style={{
    ...
    <div onClick={e => e.stopPropagation()} style={{
      ...
      maxWidth:"400px",width:"100%",padding:"2rem",position:"relative",
      boxShadow:"0 0 80px rgba(245,158,11,.06)",
    }}>
      ...
      <div style={{fontFamily:"'Outfit',sans-serif",fontWeight:800,fontSize:"1.15rem",color:"#e2e8f0",marginBottom:".3rem"}}>
        Buy Rooby a Coffee
      </div>
      <div style={{fontSize:".7rem",color:"#475569",lineHeight:1.75,marginBottom:"1.5rem"}}>
        Enjoyed this portfolio? Support my journey!<br/>
        Every contribution helps me build more AI tools. 🙏
      </div>

      {[
        { label:"☕ ×1", amt:99,  note:"A quick coffee" },
        { label:"☕ ×3", amt:299, note:"Keep me going!" },
        { label:"☕ ×5", amt:499, note:"You're amazing!" },
      ].map(({ label, amt, note }) => (
        <div key={amt} style={{
          display:"flex",alignItems:"center",justifyContent:"space-between",
          padding:".75rem 1rem",marginBottom:".6rem",
          border:"1px solid rgba(245,158,11,.15)",background:"rgba(245,158,11,.03)",
        }}>
          <div>
            <div style={{fontSize:".75rem",color:"#e2e8f0",fontWeight:600}}>{label} — ₹{amt}</div>
            <div style={{fontSize:".6rem",color:"#475569",marginTop:".15rem"}}>{note}</div>
          </div>
          <div style={{display:"flex",gap:".4rem"}}>
            {[
              { name:"GPay",    href:gpayLink(amt),    color:"#4285F4" },
              { name:"PhonePe", href:phonepeLink(amt), color:"#5f259f" },
              { name:"UPI",     href:upiLink(amt),     color:"#f59e0b" },
            ].map(btn => (
              <a key={btn.name} href={btn.href} style={{
                fontSize:".55rem",padding:".28rem .55rem",
                background:"transparent",border:`1px solid ${btn.color}40`,
                color:btn.color,textDecoration:"none",letterSpacing:".05em",whiteSpace:"nowrap",
              }}>{btn.name}</a>
            ))}
          </div>
        </div>
      ...
      <div style={{
        marginTop:"1rem",padding:".65rem 1rem",
        background:"rgba(0,212,255,.03)",border:"1px solid rgba(0,212,255,.1)",
        display:"flex",justifyContent:"space-between",alignItems:"center",
      }}>
        <div>
          <div style={{fontSize:".55rem",color:"#334155",letterSpacing:".1em",textTransform:"uppercase",marginBottom:".2rem"}}>UPI ID</div>
          <div style={{fontSize:".72rem",color:"#00d4ff"}}>{UPI_ID}</div>
        </div>
        <button onClick={() => { navigator.clipboard.writeText(UPI_ID); pushToast("UPI ID copied!","📋","default"); }} style={{
          background:"rgba(0,212,255,.08)",border:"1px solid rgba(0,212,255,.2)",
          color:"#00d4ff",fontSize:".6rem",padding:".3rem .7rem",cursor:"pointer",
        }}>Copy</button>
      </div>

      <div style={{marginTop:".85rem",fontSize:".58rem",color:"#1e293b",textAlign:"center"}}>
